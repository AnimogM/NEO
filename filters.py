...
that match all of the desired criteria. The arguments to `create_filters`
are provided by the main module and originate from the user's command line
options.
...
class AttributeFilter:
    ...
    comparing some attribute of a close approach (or its attached NEO) to
    a reference value. It essentially functions as a callable predicate
...
class HazardousFilter(AttributeFilter):
    ...
    def get(cls, approach):
        ...
        Arguments: A CloseApproach object.
        Returns: Returns the hazardous attribute of a NearEarthObject object.
...
def create_filters(
    ...
    fillterr = []
    if date:
        fillterr.append(DateFilter(operator.eq, date))
    if start_date:
        fillterr.append(DateFilter(operator.ge, start_date))
    if end_date:
        fillterr.append(DateFilter(operator.le, end_date))
    if distance_min:
        fillterr.append(DistanceFilter(operator.ge, distance_min))
    if distance_max:
        fillterr.append(DistanceFilter(operator.le, distance_max))
    if velocity_min:
        fillterr.append(VelocityFilter(operator.ge, velocity_min))
    if velocity_max:
        fillterr.append(VelocityFilter(operator.le, velocity_max))
    if diameter_min:
        fillterr.append(DiameterFilter(operator.ge, diameter_min))
    if diameter_max:
        fillterr.append(DiameterFilter(operator.le, diameter_max))
    if hazardous is not None:
        fillterr.append(HazardousFilter(operator.eq, hazardous))
...
def limit(iterator, n=None):
    ...
    """
    if n:
        return [k for j, k in enumerate(iterator) if j < n]
    return iterator
