...
def write_to_csv(results, filename):
    ...
    with open(filename, 'w') as file:
        headerr = csv.DictWriter(file, fieldnames=fieldnames)
        headerr.writeheader()
        for item in results:
            headerr.writerow(item.serial())
...
def write_to_json(results, filename):
    ...
    :param results (tuple): An iterable of `CloseApproach` objects.
    :param filename  (str): A Path-like object pointing to where the data
    should be saved.
    """
    arr = []
    for item in results:
        app = item.serial()
        app["neo"] = item.neo.serial()
        arr.append(app)
